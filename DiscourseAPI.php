...
class DiscourseAPI
{
    ...
    private function _getRequest($reqString, $paramArray = null, $apiUser = 'tomas.ulej')
    {
        if ($paramArray == null) {
            $paramArray = array();
        }
    ...
    private function _putpostRequest($reqString, $paramArray, $apiUser = 'system', $putMethod = false)
    {
        $ch = curl_init();
        $url = sprintf(
            '%s://%s%s?api_key=%s&active=true&api_username=%s',
            $this->_protocol, 
            $this->_dcHostname, 
            $reqString, 
            $this->_apiKey, 
    ...
    function createUser($name, $userName, $emailAddress, $password)
    {
        ...
        $params = array(
            ...
            //x'password_confirmation' => $obj->apiresult->value,
            'active=' => 'true'
        );
