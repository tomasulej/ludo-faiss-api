...
$q=mysql_query($query);
 ...
 while ($prislovie=mysql_fetch_object($q)) {
    ...
    if ($prislovie->klu_id>0) {
      ...
      $kluc_slova[$prislovie->klu_id].=sprintf('<div class="category">%s</div>',$utvary[$prislovie->utv_id]);    
...
*/
  ...
  $tag_array=explode(",", $kluc->txt);
    ...
    foreach ($tag_array as $value) {
		...
		$tags.=sprintf('<span class="label label-default" style="text-transform: uppercase;">%s</span> ', $value);
    }
...
//VYPIS NA STRANKU
  foreach ($arrkat as $key=>$value) {
   if ($value<>$_GET['id']) $obsah.="<section id='kategoria_".$value."'><H3>".kat_namebyid($value)."</H3>";
   if ($kapitoly[$value]<>"") {$obsah.="<ul>".$kapitoly[$value]."</ul>";}  
   if ($kluc_kap[$value]<>"") {$obsah.="<ul>".$kluc_kap[$value]."</ul></section>";} 
