...
$q1_h=mysql_query(sprintf("SELECT * FROM %s WHERE emotion<>0 ORDER BY count DESC LIMIT %s;", $tabulka1, $konst_pocet));
...
$q2_h=mysql_query(sprintf("SELECT * FROM %s WHERE emotion<>0", $tabulka2));
...
$q3_h=mysql_query(sprintf("SELECT * FROM %s WHERE emotion<>0;", $tabulka3));
...
$q4_h=mysql_query(sprintf("SELECT * FROM %s WHERE emotion<>0;", $tabulka4));
...
?>        
      ...
      var options = {
        ...
        chart: {
          title: 'Percento nájdených pozitívnych a negatívnych slov',
        },
        hAxis: {
...
<body>
  <div id="chart_div" style="height:<?php echo $konst_pocet*35;?>px"><div>
</body>
