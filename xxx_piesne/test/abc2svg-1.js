...
function Abc(user) {
    ...
    function deco_cnv(a_dcn, s, prev) {
        ...
        for (i = 0; i < n; i++) {
            ...
            switch (dd.func) {
                default: if (dd.name.slice(0, 5) == "head-") {
                    if (!s.notes) {
                ...
                case 34:
                        ...
                        break
                    }s.trem2 = true;s.beam_end = true;prev.trem2 = true;prev.beam_st = true;s.ntrem = prev.ntrem = Number(dd.name[4]);s.nflags--;prev.nflags--;
                    if (s.nflags > 0) {
                        s.nflags += s.ntrem;
                        prev.nflags += s.ntrem
                    } else {
                ...
                switch (a[0]) {
                    ...
                    case "voice":
                        ...
                        while (1) {
                            ...
                            a = text.split(/\s+/, 1);
                            switch (a[0]) {
                                default: opt[select].push(uncomment(text.trim(), true));
                                continue;
                                case "score":
                                        case "staves":
                                        case "tune":
    ...
    function gchord_width(s, wlnote, wlw) {
        ...
        for (var ix = 0; ix < s.a_gch.length; ix++) {
            ...
            switch (gch.type) {
                default: wl = -gch.x;
                if (wl > lspc) lspc = wl;w = gch.w + 2 - wl;
                if (w > rspc) rspc = w;
                break;
                case "<":
                        w = gch.w + wlnote;
    ...
    function set_yval(s) {
        switch (s.type) {
            case CLEF:
                ...
                s.y = (s.clef_line - 1) * 6;
                switch (s.clef_type) {
                    default: s.ymx = s.y + 28;s.ymn = s.y - 14;
                    break;
                    case "c":
                            s.ymx = s.y + 13;s.ymn = s.y - 11;
                        break;
                    case "b":
                            s.ymx = s.y + 7;s.ymn = s.y - 12;
                        break
                }
                if (s.clef_small) {
                    s.ymx -= 2;
    ...
    function identify_note(s, dur) {
        ...
        if (flags >= 0) {
            ...
            default: parse.line.error("Note too long");flags = -4;
            case -4:
                    head = "square";
                break;
            case -3:
    ...
    function sort_all() {
        var s, s2, p_voice, v, time, w, wmin, ir, multi, prev, new_sy, fl, nb, nv, vtb = [],
            vn = [],
            mrest_time = -1;
        nv = voice_tb.length;
    ...
    function draw_all_lyrics() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            nly_tb[v] = nly;
            if (nly == 0) continue;
            if (p_voice.pos.voc != 0) above_tb[v] = p_voice.pos.voc == SL_ABOVE;
    ...
    function gch_build(s) {
        ...
        var src, gch, w, xspc, x_abs, y_abs, ix, n, pos = curvoice.pos.gch == SL_BELOW ? -1 : 1,
        ...
        while (1) {
            ...
            gch.type = antype;
            switch (antype) {
                default: if (pos < 0) break;y_above += h_gch;
                if (box) y_above += 2;
                break;
                case "^":
                        y_above += h_ann;
    ...
    function draw_gchord(s, gchy_min, gchy_max) {
        ...
        for (ix = 0; ix < n; ix++) {
            ...
            switch (gch.type) {
                ...
                case "@":
                    y = gch.y + yav;
                    if (y > 0) y_set(s.st, 1, x, 1, y + h * .8 + 3);
                    else y_set(s.st, 0, x, 1, y - h * .2);
                    break
            }
