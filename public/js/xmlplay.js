//~ Revision: 81, Copyright (C) 2016-2018: Willem Vree, contributions Stéphane David.
...
function doLayout (abctxt) {
    ...
    function getNote (event) {
        ...
        yoff = abcElm.getBoundingClientRect ().top - abcElm.scrollTop;
...
function setYSvgs () {  // calculate top y-coordinates of all svg's (systems)
    ...
    var ynot = abcElm.getBoundingClientRect ().top - abcElm.scrollTop;
    ySvgs = deSvgs.map (function (svg) {
        return svg.getBoundingClientRect ().top - ynot;
    });
}
...
function alignSystem (isvg) {   // uitlijnen balken met de rollijn
    ...
    var u = abcElm.getBoundingClientRect ().top;
    var tmargin = t + dottedHeight - u;
    var istf = curStaff;
    var y = (stfPos [isvg][istf] - stfHgt [istf]) / gScale;
    abcElm.scrollTop = ySvgs [isvg] + y - tmargin;
    isvgAligned = isvg;
}
...
function zend (midiMsg, tijd, vce) {
    ...
    if (mtype == 0x90) {
        if (vol > 0) neer (midiNum, vol, tijd, vce);
        else op (midiNum, tijd);
    }
}

function neer (midiNum, vol, time, vce) {
...
function op (midiNum, time) {
    var x = liggend [midiNum], source = x[0], g = x[1], vol = x[2];
    if (source) {
        g.gain.setValueAtTime (vol, time);       // begin release at end of note
